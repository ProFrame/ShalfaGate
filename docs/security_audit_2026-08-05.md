...
Until that tenant-aware schema exists, the new private-signature read policy is limited to authenticated callers and paths belonging to a real, non-deleted user. It is no longer anonymous/public, but it is not yet a per-tenant authorization boundary; that final tightening belongs in the staged legacy-to-multitenant migration.
